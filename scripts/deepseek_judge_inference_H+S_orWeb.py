...
"""
使用"Highly Relevant"+"Somewhat Relevant"的文本
若没有H且没有S，使用web search检索的知识文本
提示词提示按照重要性排序
没有None
"""

def data_preprocess(file_path, web_file, n_docs):
    # 定义有效的 Relevance 值
    ...
    with open(web_file, "r", encoding="utf-8") as file:
        web_list = [line.strip() for line in file if line.strip()]
    ...
    for entry in entries:
        ...
        if query_only is None:
            ...
            merged_list = passage_list + passage_list2
            if merged_list:
                documents.append("\n".join(merged_list))
            else:
                # print(query_only)
                web_index = queries.index(query_only)
                documents.append(web_list[web_index])
            passage_list = []
            passage_list2 = []
            passage_list3 = []
            query_only = query
...
def main():
    ...
    parser.add_argument('--web_knowledge_path', type=str)
    ...
    queries, passages = data_preprocess(args.input_file, args.web_knowledge_path, args.ndocs)
