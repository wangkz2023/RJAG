...
加载生成器和评估器模型。
进行数据预处理，获取查询和文档片段。
基于指定的检索方法（rag、crag、no_retrieval）生成文档或评估结果。
调用 format_prompt 和 inference 生成预测结果。
将最终的生成结果输出到文件。
"""
...
def main():
    ...
    if args.method == 'rag':
        ...
        with open(args.internal_knowledge_path, 'r') as in_f, open(args.external_knowledge_path, 'r') as ex_f, open(
                args.combined_knowledge_path, 'r') as comb_f:
