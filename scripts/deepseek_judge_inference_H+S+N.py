...
def format_prompt(task, question, paragraph=None):
    ...
    if instruction == question:  # 如果 instruction 仍然等于 question（即没有提供任务说明）
        # PopQA
        prompt = "Refer to the following documents, documents sorted by relevance, follow the instruction and answer the question.\n\nDocuments: " + paragraph + "\n\nInstruction: Answer the question: " + question
        # Refer to the following documents, documents sorted by relevance, follow the instruction and answer the question.
        #
...
"""
使用"Highly Relevant"+"Somewhat Relevant"+"Not Relevant"的文本
提示词提示按照重要性排序
其他的使用None代替(没有None)
"""
...
def data_preprocess(file_path, n_docs):
    ...
    for entry in entries:
        ...
        if query_only is None:
            ...
            if passage_list:
                documents1 = []
                documents1.extend([content for content in passage_list])
                if passage_list2:
                    documents1.extend([content for content in passage_list2])
                if passage_list3:
                    documents1.extend([content for content in passage_list3])
                documents.append("\n".join(documents1))
            elif passage_list2:
                # print(query_only)
                documents2 = []
                documents2.extend([content for content in passage_list2])
                if passage_list3:
                    documents2.extend([content for content in passage_list3])
                documents.append("\n".join(documents2))
            elif passage_list3:
                # print(query_only)
                documents.append("\n".join(passage_list3))
    ...
    if passage_list:
        documents1 = []
        # 将 passage_list 中的元素以 " [SEP] " 连接后加入 documents
        documents1.extend([content for content in passage_list])
        # 将 passage_list2 中的元素以 " [SEP] " 连接后加入 documents
        if passage_list2:
            documents1.extend([content for content in passage_list2])
        if passage_list3:
            documents1.extend([content for content in passage_list3])
...
def main():
    ...
    # sampling_params = SamplingParams(temperature=0.0, top_p=1.0, max_tokens=500, skip_special_tokens=False)
